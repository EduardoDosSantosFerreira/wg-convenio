    ...
    try {
      this.filteredPosts = this.posts.filter((post) => {
        const matchesSearch =
          !this.currentSearch ||
          (post.titulo &&
            post.titulo.toLowerCase().includes(this.currentSearch)) ||
  ...
  createPostCard: function (post) {
    ...
    return `
      <div class="post-card" data-id="${post.id}">
        <img src="${
          post.imagem || "https://via.placeholder.com/300x180?text=Sem+Imagem"
        }" 
...
function toggleLoading(isLoading) {
  ...
  if (isLoading) {
    ...
    btnText.textContent = postIdInput.value
      ? "Atualizar Postagem"
      : "Publicar Postagem";
